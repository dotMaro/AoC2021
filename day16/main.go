...
func versionSum(s string) int {
	...
	for offsetSum < len(s)-3 {
		bitsPackage, offset := parseNextPackage(s[lastOffset:])
		// fmt.Printf("%d - %d\n", offset, len(s))
		offsetSum += offset
		versionSum += bitsPackage.version
		offset2 := offset
		if offset2%4 != 0 {
			offset2 += 4 - offset2%4
		}
...
func parseNextPackage(s string) (bitsPackage, int) {
	...
	default: // Operator.
		...
		case '0': // Length type.
			...
			for subpackageOffset < int(length) {
				_, offset := parseNextPackage(s[22:])
